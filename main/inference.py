...
from corrosion.main.image_features import extract_image_features
...
def load_artifacts(artifacts_dir: Path) -> Artifacts:
    ...
    current_path = Path(manifest["models"]["current_corrosion"]["model_path"])
    progression_path = Path(manifest["models"]["progression"]["model_path"])
    time_path = Path(manifest["models"]["time_to_threshold"]["model_path"])
