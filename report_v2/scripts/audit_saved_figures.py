...
OUT=ROOT/'report_v2'; QA=OUT/'qa/v3'; EV=OUT/'evidence/v3'
QA.mkdir(exist_ok=True); EV.mkdir(exist_ok=True)
P=ROOT/'main_4/outputs/ultimate_load_refocus'
...
def replay(cid,fn,*args,**kwargs):
 ...
 candidate=ROOT/next(x['candidate'] for x in inv if x['id']==cid)
...
ns=funcs(ROOT/'main_3/src/visualization/plots.py',names); sns.set_theme(style='whitegrid',context='talk')
...
ns=funcs(ROOT/'main_4/src/corrosion_proxy_rul/ultimate_load_refocus.py',names); sns.set_theme(style='whitegrid')
...
pdfs={}
for cid,path in [('M3-01',ROOT/'main_3/reports/corrosion_condition_pipeline_ieee.pdf'),('M4-06',ROOT/'main_4/outputs/reports/ultimate_load_refocus_ieee.pdf')]:
 cand=ROOT/next(x['candidate'] for x in inv if x['id']==cid)
