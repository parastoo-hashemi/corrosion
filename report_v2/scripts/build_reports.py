...
for p in masters:
 ...
 write_log(OUT/'qa'/f'{p.parent.parent.name}_{p.parent.name}_build.txt',run.stdout)
 if run.returncode:print(run.stdout[-6500:]);raise SystemExit(run.returncode)
 log=p.with_suffix('.log').read_text(errors='replace')
 write_log(OUT/'qa'/f'{p.parent.parent.name}_{p.parent.name}_latex_log.txt',log)
 ...
 warnings=re.findall(r'^.*(?:Warning:|pdfTeX warning).*$',log,re.M|re.I)
 if warnings:raise RuntimeError(f'{p}: {warnings}')
