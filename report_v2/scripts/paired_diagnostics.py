...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'report_v2';ref=ROOT/'main_4/outputs/ultimate_load_refocus'
...
legacy=pd.read_csv(ROOT/'main_3/outputs/canonical_dataset.csv')
tr,te=next(GroupShuffleSplit(n_splits=1,test_size=.2,random_state=42).split(legacy,groups=legacy.specimen_id))
classical=dict(total_rows=len(legacy),total_specimens=legacy.specimen_id.nunique(),test_rows=len(te),test_specimens=legacy.iloc[te].specimen_id.nunique(),train_specimens=legacy.iloc[tr].specimen_id.nunique(),method='Reconstruct GroupShuffleSplit(test_size=0.2, random_state=42) from main/modeling.py using saved legacy specimen IDs; not a saved holdout prediction manifest.')
