...
DEFAULT_STATS_CSV = PROJECT_ROOT / "augmentation" / "tables" / "image_statistics_full.csv"
...
DEFAULT_FIGURE_DIR = PROJECT_ROOT / "augmentation" / "figures" / "variants"
...
def create_report(
    *,
    ...
    command = (
        "python augmentation/create_dataset_variants.py "
...
The original images, original Excel workbook, existing delivered augmented dataset, existing split manifests, model code, and `augmentation/augment_dataset.py` were intentionally excluded from modification.
...
Brightness sigma is based on measured source-image luminance from `mean_luminance_rec709_0_255` in `augmentation/tables/image_statistics_full.csv`.
...
Model training, model selection, hyperparameter tuning, ResNet, ViT, Transformer, EfficientNet, split-manifest updates, edits to original images, edits to the original Excel workbook, edits to the existing delivered augmented dataset, and edits to `augmentation/augment_dataset.py` were excluded because this task is limited to data augmentation and dataset preparation.
