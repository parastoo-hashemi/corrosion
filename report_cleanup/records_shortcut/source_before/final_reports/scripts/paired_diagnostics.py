...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'final_reports';ref=ROOT/'structural_capacity/outputs/ultimate_load_refocus'
...
(OUT/'records/evidence/paired_diagnostics.json').write_text(json.dumps(result,indent=2,default=int)+'\n')
...
prov={'figure':str(p.relative_to(ROOT)),'script':'final_reports/scripts/paired_diagnostics.py','source_files':[str(p.relative_to(ROOT)) for p in paths.values()],'description':'Ridge metadata+HSV minus Ridge metadata mean absolute terminal error per specimen; two held-out evaluations averaged before equal-specimen aggregation. Descriptive after model selection, no significance claim.','sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'generated_utc':datetime.now(timezone.utc).isoformat()}
(OUT/'figures/PAIRED_PROVENANCE.json').write_text(json.dumps(prov,indent=2)+'\n')
(OUT/'figures/PAIRED_PROVENANCE.md').write_text('# Paired error figure provenance\n\nGenerated by `final_reports/scripts/paired_diagnostics.py` on 2026-09-16 from:\n\n'+''.join('- `'+str(p.relative_to(ROOT))+'`\n' for p in paths.values())+'\n'+prov['description']+'\n')
