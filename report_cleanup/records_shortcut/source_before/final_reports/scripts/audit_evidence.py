...
def audit():
    master_path='structural_capacity/outputs/data/master_table.csv'
    d=csv(master_path)
    ...
    auditpath='structural_capacity/outputs/audit/verified_facts.json'
    ...
    fpath='structural_capacity/outputs/features/image_features.csv'
    f=csv(fpath)
    ...
    for p,name in [('classical_corrosion/reports/current_corrosion_metrics.csv','classical'),('image_embeddings/reports/model_comparison.csv','embedding')]:
    ...
    predpath='image_embeddings/reports/predictions_best_model.csv'
    x=csv(predpath)
    ...
    legacy='condition_assessment/outputs/canonical_dataset.csv';legacy_df=csv(legacy)
    ...
    ratios=[]; early=[]
    for p in ['condition_assessment/outputs/corrosion_regression_metrics.csv','condition_assessment/outputs/damage_regression_metrics.csv']:
        x=csv(p)
    ...
    robustpath='structural_capacity/outputs/diagnostics/tables/benchmark_best_model_robustness.csv'
    ...
    claim('Fixed-model robustness ratios',ratios,[robustpath,'condition_assessment/outputs/corrosion_regression_metrics.csv','condition_assessment/outputs/damage_regression_metrics.csv'],'Regime MAE / same model grouped MAE; each generation independently normalized.')
    bestpath='structural_capacity/outputs/models/hidden_damage/best_models.csv'
    ...
    for target,model in [('peak_rust_pct','RandomForest'),('surface_total_rust_pct','GradientBoosting')]:
        p=f'structural_capacity/outputs/models/surface/{target}/leave_one_treatment_out/{target}_fold_metrics.csv'
    ...
    manifests=list((ROOT/'structural_capacity/outputs/splits').glob('master_*.csv'))
    ...
    proxy='structural_capacity/outputs/models/proxy_rul/proxy_rul_estimates.csv';x=csv(proxy)
    ...
    pjson='structural_capacity/outputs/models/proxy_rul/proxy_rul_summary.json'
    ...
    degpath='structural_capacity/outputs/models/degradation/degradation_best_fits.csv'
    deg=csv(degpath)
    ...
    register('classical_corrosion/artifacts/manifest.json')
    register('structural_capacity/configs/thresholds.yaml')
    register('structural_capacity/configs/ultimate_load_refocus.yaml')
    ...
    mappingpath='structural_capacity/configs/specimen_mapping.yaml'
    ...
    hits=[]
    for folder in ['classical_corrosion','image_embeddings','condition_assessment','structural_capacity','classification_data_preparation']:
    ...
    assert not any(h['path'].startswith('classification_data_preparation/') for h in hits)
    assert all(any(h['path'].startswith(f+'/') for h in hits) for f in ['classical_corrosion','image_embeddings','condition_assessment','structural_capacity'])
    save(pd.DataFrame(hits),'current_corruption_inventory')
    cutoff=min(r['mtime_utc'] for r in hits)
    historical=[dict(path=p,**v,predates_earliest_corrupted_source=v['mtime_utc']<cutoff) for p,v in SOURCES.items() if p.startswith(('classical_corrosion/','image_embeddings/','condition_assessment/','structural_capacity/')) and '/outputs/' in p or p.startswith(('classical_corrosion/reports/','image_embeddings/reports/'))]
    ...
    (OUT/'records/evidence'/'claims.json').write_text(json.dumps(CLAIMS,indent=2,default=str)+'\n')
    ...
    (OUT/'records/VERIFICATION_LOG.md').write_text('\n'.join(log))
