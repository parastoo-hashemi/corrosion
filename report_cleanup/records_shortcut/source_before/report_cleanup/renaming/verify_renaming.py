...
REPORT_CLEANUP = ROOT / 'report_cleanup/final_reports'
REPORT_MANIFEST = json.loads((REPORT_CLEANUP/'manifest.json').read_text())
...
def report_location(path):
    if path in REPORT_MOVES:
        return REPORT_MOVES[path]
    if path.startswith('report_v2/'):
        return 'final_reports/' + path[len('report_v2/'):]
    return path

def moved(path):
    parts = Path(path).parts
    renamed = str(Path(MAPPING.get(parts[0], parts[0]), *parts[1:]))
    return report_location(SELECTED_MOVES.get(renamed, renamed))
...
def main():
    ...
    retired = {moved(row['path']): row for row in removal_manifest['removed_files']}
    ...
    report_edits = {row['path']: row['sha256'] for row in
                    json.loads((REPORT_CLEANUP/'modified_before.json').read_text())}
    report_before = json.loads((REPORT_CLEANUP/'before.json').read_text())
    report_original = {row['path']: row for row in report_before['files']}
    check(set(REPORT_MOVES).isdisjoint(REPORT_REMOVED)
          and set(REPORT_MOVES) | set(REPORT_REMOVED) == set(report_original), 'report migration coverage')
    for row in REPORT_MANIFEST['moved_files']:
        path = ROOT/row['new_path']
        check(path.is_file() and not path.is_symlink(), 'report moved file: ' + row['new_path'])
        check(row['sha256'] == report_original[row['old_path']]['sha256'],
              'report original identity: ' + row['old_path'])
        if row['old_path'] not in report_edits:
            check(path.is_file() and digest(path) == row['sha256'], 'report preserved bytes: ' + row['new_path'])
    for name, row in REPORT_REMOVED.items():
        check(Path(name).suffix in {'.aux', '.bbl', '.blg', '.fdb_latexmk', '.fls', '.log', '.out', '.toc', '.lof', '.lot'}
              or name == 'report_v2/thesis/v3/figures/README.md', 'report removal scope: ' + name)
        check(row['sha256'] == report_original[name]['sha256'] and not os.path.lexists(ROOT/name),
              'report removed-file identity: ' + name)
    ...
    check(not os.path.lexists(ROOT/'report_v2'), 'old report directory remains')
    backup = ROOT.parent/REPORT_MANIFEST['backup']['path_relative_to_repository_parent']
    result['external_final_reports_backup_present'] = backup.is_file()
    if backup.is_file():
        check(digest(backup) == REPORT_MANIFEST['backup']['sha256'], 'final reports backup integrity')
    result['report_files_relocated'] = len(REPORT_MOVES)
    result['report_build_files_removed'] = len(REPORT_REMOVED) - 1
    result['report_archived_files_hashed'] = len(report_before['archive_files'])
    ...
    for row in read('symlinks_before.json'):
        if row['path'] in REPORT_LINKS_REMOVED:
            check(row['target'] == REPORT_LINKS_REMOVED[row['path']]['target'], 'removed report shortcut identity: ' + row['path'])
            continue
    ...
    result['preserved_existing_links'] = len(read('symlinks_before.json')) - len(REPORT_LINKS_REMOVED)
    result['consolidated_report_links'] = len(REPORT_LINKS_REMOVED)

    edits = {moved(row['path']): row for row in read('edited_files.json')}
    baseline = read('files_before.json')
    result['live_workspace_metadata'] = []
    hashed = parsed = ast_matches = unchanged = 0
    for row in baseline:
        if row['path'] in REPORT_REMOVED:
            check(row['sha256'] == REPORT_REMOVED[row['path']]['sha256'], 'removed report baseline: ' + row['path'])
            continue
        name = moved(row['path']); path = ROOT / name
        if name in retired:
        ...
        edit = edits.get(name)
        if edit:
            before = ROOT/edit['before_path'] if 'before_path' in edit else OUT/'source_before'/edit['path']
        ...
        if path.suffix == '.py':
            try:
                ...
                if edit:
                    original = ast.parse((OUT / 'source_before' / edit['path']).read_text())
    ...
    if (OUT / 'added_files.json').exists():
        ...
        for row in additions:
            if row['path'] in REPORT_REMOVED:
                check(row['sha256'] == REPORT_REMOVED[row['path']]['sha256'], 'removed report addition: ' + row['path'])
                continue
            p=ROOT/moved(row['path'])
            if moved(row['path']) in retired:
                check(row['sha256'] == retired[moved(row['path'])]['sha256'],
    ...
    documents = [ROOT/'README.md', ROOT/'CLAUDE.md', ROOT/'final_reports/README.md']
    documents += [ROOT/'final_reports/scripts/README.md', publication/'README.md',
    ...
    tex_inputs=set()
    for master in [ROOT/'final_reports/sources/thesis/thesis.tex',ROOT/'final_reports/sources/article/article.tex']:
        ...
        while pending:
            ...
            for name in re.findall(r'\\fig\{([^}]+)\}',text):
                p=ROOT/'final_reports/figures'/f'{name}.pdf'
    ...
    if args.full:
        ...
        for row in phase_baseline:
            if row['path'] in REPORT_REMOVED:
                check(row['sha256'] == REPORT_REMOVED[row['path']]['sha256'], 'removed report alias-phase baseline: ' + row['path'])
                continue
            if moved(row['path']) in retired:
                continue
            ...
            recorded_edit |= report_edits.get(row['path']) == row['sha256']
            current_edit = edits.get(moved(row['path']))
            if current_edit:
