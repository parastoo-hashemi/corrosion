...
| [exploratory_prototype/](exploratory_prototype/) | Exploratory prototype; no defensible held-out benchmark |
| [classical_corrosion/](classical_corrosion/) | Historical classical corrosion baseline and API code |
| [image_embeddings/](image_embeddings/) | Historical frozen deep-image embeddings with tabular context |
| [condition_assessment/](condition_assessment/) | Interpretable features, structural feasibility, and first proxy-RUL pipeline |
| [structural_capacity/](structural_capacity/) | Most mature structural generation: robustness analysis and terminal-load refocus |
| [archive/structural_baseline_snapshot/](archive/structural_baseline_snapshot/) | Retained earlier baseline snapshot; not the current entry point |
| [classification_data_preparation/](classification_data_preparation/) | Separate four-class data preparation and specimen split scripts, plus the augmentation methodology report and dataset report |
...
The directories now describe their research purpose. The [folder migration guide](docs/folder_migration.md)
maps every old name to its current location. Small compatibility links keep old
paths usable for frozen manifests and manuscript citations; they contain no duplicate
data. The [project history](docs/project_history.md) explains the phases.
...
files are [classical_corrosion/requirements.txt](classical_corrosion/requirements.txt),
[image_embeddings/requirements.txt](image_embeddings/requirements.txt),
[condition_assessment/requirements.txt](condition_assessment/requirements.txt), and
[classification_data_preparation/requirements.txt](classification_data_preparation/requirements.txt).
They specify minimum versions, not locked historical environments.
**There is no `structural_capacity/requirements.txt`.**
...
- Most mature structural results: [structural_capacity/outputs/ultimate_load_refocus/](structural_capacity/outputs/ultimate_load_refocus/).
...
- Earlier robustness/degradation outputs: [structural_capacity/outputs/models/](structural_capacity/outputs/models/)
  and [diagnostics/](structural_capacity/outputs/diagnostics/).
...
historical `condition_assessment/src/data/` source files. A full
...
1. Review the delivered v3 thesis and article, then choose the next research question.
...
- Earlier draft versions and the original activity report are retained in
  ...
  [report guide](report_v2/README.md) for the current delivery scope.
- [Folder-renaming verification](report_cleanup/renaming/RENAMING_REPORT.md) records
  the dependency changes, preservation checks, and remaining limitations.
