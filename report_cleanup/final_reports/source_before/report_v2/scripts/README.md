...
figure generation. They are not required to read the delivered PDFs. No script
was changed or run during the report-folder navigation update.

## Purpose and output locations

| Script | Purpose | Files it can write under report_v2/ |
|---|---|---|
| [audit_evidence.py](audit_evidence.py) | Checks saved numerical claims, prediction grouping and dataset facts | CSV tables, `evidence/claims.json`, `VERIFICATION_LOG.md` |
| [paired_diagnostics.py](paired_diagnostics.py) | Compares saved specimen errors for metadata-only and metadata+HSV Ridge | Paired CSV/LaTeX statistics, paired figure/provenance, `evidence/paired_diagnostics.json` |
...
| [make_v3_figures.py](make_v3_figures.py) | Produces the three additional v3 Ridge diagnostics from saved results | Ridge PDF/PNG figures, main provenance, `evidence/v3/new_figure_checks.json` |
| [audit_saved_figures.py](audit_saved_figures.py) | Audits saved figure inputs and replays selected plotting functions | `qa/v3/` replay outputs and `evidence/v3/figure_checks.json` |
...
`evidence/`, `qa/` and `VERIFICATION_LOG.md` are archive links: writes through them
also change the linked archive records. Use a separate complete repository copy
