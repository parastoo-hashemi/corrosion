...
def finish(fig,name,sources,description):
    ...
    ledger.append({'figure':str(p.relative_to(ROOT)),'script':'final_reports/scripts/make_figures.py','source_files':sources,'description':description,'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'generated_utc':datetime.now(timezone.utc).isoformat()})
...
notes=['# Figure provenance','','All figures are generated by `final_reports/scripts/make_figures.py` from the exact source paths below. Run date: 2026-09-16. Every numerical input is checked by `audit_evidence.py`; schematic geometry is illustrative. PDF is the publication master; PNG is a viewing copy.','']
