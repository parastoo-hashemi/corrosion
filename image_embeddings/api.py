...
from corrosion.image_embeddings.inference import Phase2Artifacts, load_phase2_artifacts, predict_current_corrosion
...
def get_artifacts() -> Phase2Artifacts:
    ...
    if not (artifacts_dir / "run_info.json").exists():
        raise RuntimeError(
            "Phase 2 artifacts not found. Run: python -m corrosion.image_embeddings.train_phase2"
        )
